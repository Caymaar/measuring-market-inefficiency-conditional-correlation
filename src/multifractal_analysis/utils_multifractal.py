...
class ComputeMFDFA:
    @staticmethod
    def mfdfa(
        signal: Union[np.ndarray, Sequence[float]], scales: Sequence[int], q_list: Sequence[float], order: int = 1
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def compute_alpha_falpha(q_list: Sequence[float], h_q: Sequence[float]) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    @staticmethod
    def surrogate_gaussian_corr(series: Sequence[float]) -> np.ndarray:
        """
