...
class AggregateVarianceHurstEstimator(AbstractHurstEstimator, AddMethods):
    """
    Hurst exponent estimator using the Aggregate Variance method.
    """

    def __init__(self, time_series: np.ndarray, minimal: int = 20,
                 method: str = 'L2'):
        """
        Initialize the Aggregate Variance estimator.
    ...
    def estimate(self) -> float:
        ...
        for m in scales:
            ...
            variances.append(np.var(block_means, ddof=0))
        slope = self._fit_log_log(scales, variances)
        return (slope + 2) / 2
